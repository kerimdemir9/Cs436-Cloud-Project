...
import static com.customer.transaction.validation.Constants.*;
...
public class TransactionValidatorImpl implements TransactionValidator {
    ...
    public void validate(TransactionModel model) {
        notNullObject.test(model.getReceiver().getId()).throwIfInvalid(RECEIVER_ID_FIELD_FOR_VALIDATION);
        notNullObject.test(model.getSender().getId()).throwIfInvalid(SENDER_ID_FIELD_FOR_VALIDATION);
